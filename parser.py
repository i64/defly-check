...
class Parser():
    ...
    def get_str(data: DataView, offset: int):
        ...
        for counter in range(size):
            result += chr(data.get_uint_8(offset + 2 * counter + 1) |
                          data.get_uint_8(offset + 2 * counter + 0) << 8)
    ...
    def parse_user(_data):
        ...
        username = Parser.get_str(data, 5)
    ...
    def parse_teams(_data: bytes):
        ...
        for _ in range(team_count):
            ...
            for member_id in range(member_limit):
                ...
                iter_counter += 4
            teams.append(dict(team_id=team_id, map_percent=map_percent,
                              available=available, members=members))
